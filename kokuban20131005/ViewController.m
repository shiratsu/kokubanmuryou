...
- (void)viewDidLoad
{
    ...
    billingMode = [defaults boolForKey:@"billingmode"];
...
-(void)showBuyConfirmAlert{
    ...
    [alert setTitle:@"購入確認"];
    [alert setMessage:@"こくばん Proを購入しますか？？"];
    [alert setDelegate:self];
    [alert addButtonWithTitle:@"ok"];
    [alert addButtonWithTitle:@"cancel"];
    [alert addButtonWithTitle:@"二度と購入確認を行わない"];
    [alert show];
}
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    switch (buttonIndex) {
        ...
        case 2:
            billingMode = TRUE;
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setBool:billingMode forKey:@"billingmode"];
            break;
...
-(void)gotoAppStore{
    NSURL *url = [NSURL URLWithString:@"https://itunes.apple.com/jp/app/kokuban-pro/id818934135?mt=8"];
